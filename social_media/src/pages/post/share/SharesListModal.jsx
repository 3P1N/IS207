...
export default function SharesListModal({ postId, onClose }) {
    
    // --- 2. SỬ DỤNG USEQUERY ---
    const { data: shares = [], isLoading } = useQuery({
        queryKey: ['shares', postId], // Key theo postId
        queryFn: async () => {
        ...
        >
            ...
            >
                ...
                <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h3 className="font-bold text-lg text-gray-800 dark:text-gray-100">Shares</h3>
                ...
                <div className="overflow-y-auto p-4 flex-1">
                    {isLoading ? (
                        ...
                        <p className="text-center text-gray-500 py-4">No one has shared this post yet.</p>
                    ) : (
                        <div className="flex flex-col gap-3">
                            {shares.map((share) => (
                                <div key={share.id || share.user.id} className="flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded-lg transition-colors cursor-pointer">
                                    {/* Avatar */}
                                    <AvatarUser userData={share.user}/>
                                    
                                    {/* Name */}
                                    <div>
                                        <p className="font-semibold text-gray-800 dark:text-gray-200 text-sm">{share.user.name}</p>
